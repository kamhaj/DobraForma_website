...
if( ! class_exists( 'Bootstrap_Fitness_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Bootstrap_Fitness_Control_Repeater extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'bootstrap-fitness' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'bootstrap-fitness' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'bootstrap-fitness-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'bootstrap-fitness-repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'bootstrap-fitness' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    	...
    	public function repeater_js_template() {
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row=0>
    					...
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'bootstrap-fitness' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="bootstrap-fitness-image-attachment" data-placeholder="<?php echo esc_attr( 'No Image Selected', 'bootstrap-fitness' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php echo esc_attr( 'No Image Selected', 'bootstrap-fitness' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_html( 'Remove', 'bootstrap-fitness' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr( 'Add Image', 'bootstrap-fitness' ); ?>" data-alt-label="<?php echo esc_attr( 'Change Image', 'bootstrap-fitness' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change Image', 'bootstrap-fitness' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add Image', 'bootstrap-fitness' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="bootstrap-fitness-file-attachment" data-placeholder="<?php echo esc_attr( 'No File Selected', 'bootstrap-fitness' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php echo esc_attr( 'No File Selected', 'bootstrap-fitness' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php echo esc_attr( 'Add File', 'bootstrap-fitness' ); ?>" data-alt-label="<?php echo esc_attr( 'Change File', 'bootstrap-fitness' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php echo esc_attr( 'Change File', 'bootstrap-fitness' ); ?>
    										<# } else { #>
    											<?php echo esc_attr( 'Add File', 'bootstrap-fitness' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
